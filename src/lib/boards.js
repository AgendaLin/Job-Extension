...
// 每次都搜的板。
//
// 嚴格說只有 Salary（跨領域薪水）和 job（通用求職）是真正通用的；
// Tech_Job／Soft_Job 是科技專屬，照理不該對所有產業搜。但實測推翻了這個直覺——
// 工程師在每個產業都有，會在這兩個板討論自己的雇主：
//   長庚(醫院) Soft_Job 18 筆、華航 Tech_Job 20 + Soft_Job 13、國泰人壽 Soft_Job 13
// 拿掉會損失真實結果，而留著的代價只是對零售/會計那類多送 2 個空請求（全聯 0/4、資誠 0/1）。
// 所以留著是資料支持的決定，不是偷懶。
...
const INDUSTRY_RULES = [
  ...
  // 一個產業可以對到多個板：醫療在護理板和醫護板都熱門
  // 實測：長庚/台大醫院/馬偕/榮總 在 Nurse 各 20 筆；長庚/台大醫院 在 Doctor-Info 各 20 筆
  { keywords: ["醫院", "診所", "醫療保健"], boards: ["Nurse", "Doctor-Info"] },
  // 國營事業自成一個板，這整個族群原本完全沒搜到。
  // 實測：台電/中油/台水/中華郵政 在 Gov_owned 各 20 筆（滿頁）。
  // 關鍵字不能只寫「電力」「水」——會誤中「電力機械器材製造修配業」「水泥製品製造業」等製造業。
  {
    keywords: ["電力供應", "用水供應", "郵政", "石油煉製", "石油／天然氣"],
    boards: ["Gov_owned"],
  },
