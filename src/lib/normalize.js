...
// 組織型態「前綴」。台灣很多雇主的型態字擺在前面，只砍後綴會整串搜而得到 0 筆，要從頭砍：
// - 法人/公立機構：財團法人工業技術研究院 → 工業技術研究院
// - 外商：104 慣例是「{國}商{品牌}」，例：新加坡商蝦皮娛樂電商 → 蝦皮娛樂電商
const ORG_PREFIXES = [
  "財團法人", "社團法人", "行政法人", "國立", "私立",
  "美商", "日商", "新加坡商", "香港商", "英商", "德商", "荷商",
  "法商", "韓商", "瑞士商", "澳商", "加拿大商",
];
...
const INDUSTRY_WORDS = [
  ...
  "證券", "通訊", "生醫",
  // 稽核（scripts/audit.mjs）補：品牌後常見的業別字
  "廣告", "餐飲", "電視", "傳播", "媒體", "影視", "航空", "百貨",
  "旅行社", "人壽", "產物", "商業銀行",
  // 第二批稽核補：電商 / 飯店
  "娛樂電商", "電商", "國際酒店", "酒店", "大飯店", "飯店", "觀光",
]
  // 由長到短排序：確保「商業銀行」比「銀行」先被比對到，否則「玉山商業銀行」只會被剝成
  // 「玉山商業」。排序後就不必煩惱手動維護順序。
